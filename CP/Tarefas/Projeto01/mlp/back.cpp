...
void Neuronio ::Inicializar_Neuronio(int Numero_Pesos)
{
  ...
  for (i = 0; i < Numero_Pesos; i++)
    W[i] = (double)(rand()%11 / 10.0) * (random(3) - 1.0);
}
...
double Neuronio ::Somatorio(double Entrada[])
{
  int i;
  double Som = 0;
  #pragma omp paralle for reduction(+:Som)
  for (i = 0; i < Numero_Pesos; i++)
    Som += Entrada[i] * W[i];
                             ...
                             int Numero_Colunas_Entrada, int Numero_Colunas_Saida, int Numero_Neuronio_Camada[])
{
  ...
  Entrada = fopen("X-text.txt", "r");
  Saida = fopen("Y-text.txt", "r");

  for (i = 0; i < Numero_Linhas; i++){
    for (j = 0; j < Numero_Colunas_Entrada; j++){
      fscanf(Entrada, "%lf", &X[i][j]);
...
int main()
{
  ...
  while (Continua != 'n')
  {
    if (Continua == 'r')
    {
      ...
      printf("\nTime Inicializar Rede= %f", end - start);
      start = omp_get_wtime();
      R.Treinar();
      end = omp_get_wtime();
      printf("\nTime Treinar Rede= %f", end - start);
    }
